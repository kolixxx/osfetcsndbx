...
# Установка опциональных модулей (рекомендуется)
pkg install p5-IO-KQueue      # для kqueue (эффективный мониторинг файлов)
pkg install p5-File-LibMagic  # для определения типа файла по содержимому
```

**Примечание:** Если пакеты `p5-IO-KQueue` или `p5-File-LibMagic` недоступны в репозитории OPNsense, скрипт будет работать в режиме polling и использовать пакет `exe` по умолчанию.
